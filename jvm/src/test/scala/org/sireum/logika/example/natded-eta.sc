...
@pure def introNatDed(p: B, q: B, r: B): Unit = {
  Deduce(
    ...
    Proof(
      ...
      3 #> SubProof(
        4 #> Assume(p),
        5 #> (p & r)           by andI _ and (4, 2),
        6 #> (p & r | q & r)   by orI1 _ and 5
      ),
      7 #> SubProof(
        8 #> Assume(q),
        9 #> (q & r)           by andI _ and (8, 2),
        10 #> (p & r | q & r)  by orI2 _ and 9
      ),
...
@pure def and1(p: B, q: B, r: B): Unit = {
  Deduce(
    ...
    Proof(
      ...
      2 #> q               by Premise,
      3 #> r               by Premise,
      4 #> (q & p)         by andI _ and (2, 1),
      5 #> (r & (q & p))   by andI _ and (3, 4),
    )
    //@formatter:on
  )
}
...
@pure def and2(p: B, q: B, r: B): Unit = {
  Deduce(
    ...
    Proof(
      1 #> (p & (q & r))   by Premise,
      2 #> p               by andE1 _ and 1,
      3 #> (q & r)         by andE2 _ and 1,
      4 #> r               by andE2 _ and 3,
      5 #> (r & p)         by andI _ and (4, 2),
    )
    //@formatter:on
  )
}
...
@pure def or2b(p: B, q: B): Unit = {
  Deduce(
    ...
    Proof(
      1 #> (p & q)   by Premise,
      2 #> q         by andE2 _ and 1,
      3 #> (p | q)   by orI2 _  and 2,
    )
    //@formatter:on
  )
}
...
@pure def imply1(p: B, q: B, r: B): Unit = {
  Deduce(
    ...
    Proof(
      ...
      2 #> (p ->: q)             by Premise,
      3 #> p                     by Premise,
      4 #> q                     by implyE _ and (2, 3),
      5 #> (p & q)               by andI _ and (3, 4),
      6 #> r                     by implyE _ and (1, 5),
    )
    //@formatter:on
  )
}
  ...
  Deduce(
    ...
    Proof(
      ...
      3 #> SubProof(
        ...
        5 #> SubProof(
          6 #> Assume(p),
          7 #> r                 by implyE _ and (1, 6)
        ),
        8 #> SubProof(
          9 #> Assume(q),
         10 #> r                 by implyE _ and (2, 9)
        ),
        11 #> r                  by OrE(4, 5, 8)
      ),
...
@pure def negation1(p: B, q: B): Unit = {
  Deduce(
    ...
    Proof(
      ...
      3 #> SubProof(
        4 #> Assume(p),
        5 #> F            by negE _ and (4, 2),
        6 #> q            by bottomE _ and 5
      ),
      7 #> SubProof(
        8 #> Assume(q)
      ),
...
@pure def negation2(p: B, q: B): Unit = {
  Deduce(
    ...
    Proof(
      ...
      2 #> SubProof(
        3 #> Assume(p),
        4 #> F             by negE _ and (3, 1),
        5 #> q             by bottomE _ and 4
      ),
      6 #> (p ->: q)       by ImplyI(2),
    )
    //@formatter:on
...
@pure def negation3(p: B, q: B): Unit = {
  Deduce(
    ...
    Proof(
      ...
      2 #> SubProof(
        3 #> Assume(p & q),
        4 #> p                by andE1 _ and 3,
        5 #> q                by andE2 _ and 3,
        6 #> (!q)             by implyE _ and (1, 4),
        7 #> F                by negE _ and (5, 6)
      ),
      8 #> (!(p & q))         by NegI(2),
    )
    //@formatter:on
...
@pure def negation4(p: B, q: B): Unit = {
  Deduce(
    ...
    Proof(
      ...
      3 #> SubProof(
        4 #> Assume(q),
        5 #> (!p)         by implyE _ and (2, 4),
        6 #> F            by negE _ and (1, 5)
      ),
      7 #> (!q)           by NegI(3),
    )
    //@formatter:on
...
@pure def negation7(p: B, q: B): Unit = {
  Deduce(
    ...
    Proof(
      ...
      2 #> SubProof(
        3 #> Assume(!p),
        4 #> (!p | !q)     by orI1 _ and 3,
        5 #> F             by negE _ and (4, 1)
      ),
      6 #> p               by PbC(2),
      7 #> SubProof(
        8 #> Assume(!q),
        9 #> (!p | !q)     by orI2 _ and 8,
       10 #> F             by negE _ and (9, 1)
      ),
      11 #> q              by PbC(7),
      12 #> (p & q)        by andI _ and (6, 11)
    )
    //@formatter:on
  )
}
  ...
  Deduce(
    ...
    Proof(
      1 #> SubProof(
        ...
        3 #> SubProof(
          4 #> Assume(p),
          5 #> (p | !p)           by orI1 _ and 4,
          6 #> F                  by negE _ and (5, 2)
        ),
        7 #> (!p)                 by NegI(3),
        8 #> (p | !p)             by orI2 _ and 7,
        9 #> F                    by negE _ and (8, 2)
      ),
      10 #> (p | !p)              by PbC(1),
    )
    //@formatter:on
...
@pure def universal1[T](human: T => B @pure, mortal: T => B @pure, Socrates: T): Unit = {
  Deduce(
    ...
    Proof(
      ...
      3 #> (human(Socrates) ->: mortal(Socrates))     by allE[T] _ and 1,
      4 #> mortal(Socrates)                           by implyE _ and (3, 2),
    )
    //@formatter:on
  )
}
...
@pure def universal2[T](gt: (T, T) => B @pure, inc: T => T @pure, dec: T => T @pure): Unit = {
  Deduce(
    ...
    Proof(
      ...
      3 #> Let { (a: T) => SubProof(
        4 #> gt(inc(a), a)                              by allE[T] _ and 1,
        5 #> gt(a, dec(a))                              by allE[T] _ and 2,
        6 #> (gt(inc(a), a) & gt(a, dec(a)))            by andI _ and (4, 5)
      )},
  ...
  Deduce(
    ...
    Proof(
      ...
      3 #> Let { (a: T) => SubProof(
        4 #> SubProof(
          5 #> Assume(human(a)),
          6 #> (human(a) ->: mortal(a))          by allE[T] _ and 1,
          7 #> mortal(a)                         by implyE _ and (6, 5),
          8 #> (mortal(a) ->: soul(a))           by allE[T] _ and 2,
          9 #> soul(a)                           by implyE _ and (8, 7)
        ),
  ...
  Deduce(
    ...
    Proof(
      ...
      2 #> SubProof(
        ...
        4 #> Let { (a: T) => SubProof(
          5 #> healthy(a)                                         by allE[T] _ and 3,
          6 #> (healthy(a) ->: happy(a))                          by allE[T] _ and 1,
          7 #> happy(a)                                           by implyE _ and (6, 5)
        )},
...
@pure def existential1[T](human: T => B @pure, mortal: T => B @pure, Socrates: T): Unit = {
  Deduce(
    ...
    Proof(
      ...
      3 #> (human(Socrates) & mortal(Socrates))   by andI _ and (1, 2),
      4 #> ∃{(x: T) => human(x) & mortal(x)}      by existsI[T] _ and 3
    )
    //@formatter:on
  )
}
...
@pure def existential2(vowel: C => B @pure, square: (Z, Z) => C @pure, holds: (C, C) => B @pure, e: C): Unit = {
  Deduce(
    ...
    Proof(
      ...
      3 #> ∃{(x: C) => holds(x, e)}                           by existsI[C] _ and 2,
      4 #> (vowel(e) & ∃{(x: C) => holds(x, e)})              by andI _ and (1, 3),
      5 #> ∃{(y: C) => vowel(y) & ∃{(x: C) => holds(x, y)}}   by existsI[C] _ and 4
    )
    //@formatter:on
  )
}
...
@pure def existential3(vowel: C => B @pure, square: (Z, Z) => C @pure, holds: (C, C) => B @pure, e: C): Unit = {
  Deduce(
    ...
    Proof(
      1 #> vowel(e)                                    by Premise,
      2 #> holds(square(1, 4), e)                      by Premise,
      3 #> (vowel(e) & holds(square(1, 4), e))         by andI _ and (1, 2),
      4 #> ∃{(x: C) => vowel(e) & holds(x, e)}         by existsI[C] _ and 3,
      5 #> ∃{(y: C, x: C) => vowel(y) & holds(x, y)}   by existsI[C] _ and 4,
    )
    //@formatter:on
  )
}
...
@pure def existential4[T](human: T => B @pure, mortal: T => B @pure): Unit = {
  Deduce(
    ...
    Proof(
      ...
      3 #> Let { (a: T) => SubProof(
        4 #> Assume(human(a)),
        5 #> (human(a) ->: mortal(a))             by allE[T] _ and 1,
        6 #> mortal(a)                            by implyE _ and (5, 4),
        7 #> ∃{(z: T) => mortal(z)}               by existsI[T] _ and 6
      )},
  ...
  Deduce(
    ...
    Proof(
      ...
      3 #> Let { (a: Z) => SubProof(
        ...
        5 #> covered(a)                                                    by andE1 _ and 4,
        6 #> ∃{(x: Z) => covered(x)}                                       by existsI[Z] _ and 5
      )},
      7 #> ∃{(x: Z) => covered(x)}                                         by ExistsE[Z](1, 3),
      8 #> (!gameOver)                                                     by implyE _ and (2, 7),
    )
    //@formatter:on
  )
}
